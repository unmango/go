...
var _ = Describe("RootContext", Label("Dependency:Git"), func() {
	...
	It("should print the working directory's git root", func(ctx context.Context) {
		...
		p, err := git.RootContext(git.WithWorkingDirectory(ctx, subdir))
	...
	It("should use git from the context", func(ctx context.Context) {
		...
		p, err := git.RootContext(git.WithGitPath(ctx, exe))
